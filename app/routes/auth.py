...
def register():
    ...
    if form.validate_on_submit():
        ...
        with session_scope() as session:
            ...
            domicile_longitude_and_latitude = GeolocalizationService.get_coordinates(
                form.domicile.data
            )
            ...
            logger.info(
                f"User {form.username.data} registered and logged in successfully."
            )
    ...
    if form.validate_on_submit():
        ...
        with session_scope() as session:
            ...
            if user.check_user_in_db_login(form.password.data) and not user.is_admin():
                ...
                login_user(user_log)
                logger.info(
                    f"Administrator {form.username.data} logged in successfully."
                )
                return redirect(url_for("admin.admin_site"))

            else:
                logger.warning(
                    f"Failed login attempt for user: {form.username.data} (User does not exist or wrong password)."
                )
