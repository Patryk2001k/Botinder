...
from app.services.database_operations import (
    session_scope, get_chatroom_messages, add_match_unmatch, 
    delete_chatroom, delete_matched_pair, add_message_to_chatroom
)
...
class ChatService:
    ...
    def get_chatroom_context(cls, user, chatroom_id: int) -> ChatroomContextDTO:
        ...
        with session_scope() as session:
            ...
            chatroom = session.query(ChatRoom).options(
                joinedload(ChatRoom.robot).joinedload(UserRobot.profile_robot)
            ).filter_by(id=chatroom_id).first()
